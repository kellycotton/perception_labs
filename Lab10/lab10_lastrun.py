...
end_text_6 = visual.TextStim(win=win, name='end_text_6',
    text='',
    font='Open Sans',
    pos=(-.3, -.1), height=0.03, wrapWidth=None, ori=0.0, 
    color='white', colorSpace='rgb', opacity=None, 
    languageStyle='LTR',
    depth=-7.0);
end_text_7 = visual.TextStim(win=win, name='end_text_7',
    ...
    languageStyle='LTR',
    depth=-8.0);
end_text_8 = visual.TextStim(win=win, name='end_text_8',
    text='',
    font='Open Sans',
    pos=(.3, -.1), height=0.03, wrapWidth=None, ori=0.0, 
    color='white', colorSpace='rgb', opacity=None, 
    languageStyle='LTR',
    depth=-9.0);
end_text_9 = visual.TextStim(win=win, name='end_text_9',
...
for thisBlock_disc in block_disc:
    ...
    # set up handler to look after randomisation of conditions etc
    trials_disc = data.TrialHandler(nReps=2.0, method='random', 
        extraInfo=expInfo, originPath=-1,
    ...
    for thisTrials_disc in trials_disc:
        ...
        choice_cond = np.random.choice([1,2])
        
        if choice_cond == 1: # different condition
            stim_sound_2 = stimuli_sound_2
            corr_ans = "right"
        elif choice_cond == 2: # same condition
            stim_sound_2 = stimuli_sound_1
            corr_ans = "left"
        
        sound_id_2.setSound(stimuli_sound_1, hamming=True)
        sound_id_2.setVolume(1.0, log=False)
        sound_id_3.setSound(stim_sound_2, hamming=True)
        ...
        while continueRoutine:
            ...
            if key_resp_4.status == STARTED and not waitOnFlip:
                ...
                if len(_key_resp_4_allKeys):
                    ...
                    # was this correct?
                    if (key_resp_4.keys == str(corr_ans)) or (key_resp_4.keys == corr_ans):
                        key_resp_4.corr = 1
                    else:
                        key_resp_4.corr = 0
                    # a response ends the routine
        ...
        # store data for trials_disc (TrialHandler)
        trials_disc.addData('key_resp_4.keys',key_resp_4.keys)
        trials_disc.addData('key_resp_4.corr', key_resp_4.corr)
    ...
    # completed 2.0 repeats of 'trials_disc'
    ...
    print(dg_before_acc, dg_mid_acc, dg_after_acc, bd_before_acc, bd_mid_acc, bd_after_acc)
...
msg_3 = f"Discrimination\nBefore\nD-G\n {round(dg_before_acc/dg_before_trial_n, 1)}"
msg_4 = f"Discrimination\nMid\nD-G\n {round(dg_mid_acc/dg_mid_trial_n, 1)}"
msg_5 = f"Discrimination\nAfter\nD-G\n {round(dg_after_acc/dg_after_trial_n, 1)}"
msg_6 = f"B-D\n {round(bd_before_acc/bd_before_trial_n, 1)}"
msg_7 = f"B-D\n {round(bd_mid_acc/bd_mid_trial_n, 1)}"
msg_8 = f"B-D\n {round(bd_after_acc/bd_after_trial_n, 1)}"

thisExp.addData("Discrimination Before D-G ",  round(dg_before_acc/dg_before_trial_n, 1))
thisExp.addData("Discrimination Mid D-G ",  round(dg_mid_acc/dg_mid_trial_n, 1))
thisExp.addData("Discrimination After D-G ",  round(dg_after_acc/dg_after_trial_n, 1))
thisExp.addData("Discrimination Before B-D ",  round(bd_before_acc/bd_before_trial_n, 1))
thisExp.addData("Discrimination Mid B-D ",  round(bd_mid_acc/bd_mid_trial_n, 1))
thisExp.addData("Discrimination After B-D ",  round(bd_after_acc/bd_after_trial_n, 1))
...
end_text_7.setText(msg_7)
end_text_8.setText(msg_8)
key_resp_end.keys = []
key_resp_end.rt = []
_key_resp_end_allKeys = []
# keep track of which components have finished
finalComponents = [end_text, end_text_1, end_text_2, end_text_3, end_text_4, end_text_5, end_text_6, end_text_7, end_text_8, end_text_9, key_resp_end]
...
while continueRoutine:
    ...
    # *end_text_8* updates
    if end_text_8.status == NOT_STARTED and tThisFlip >= 0.0-frameTolerance:
        # keep track of start time/frame for later
        end_text_8.frameNStart = frameN  # exact frame index
        end_text_8.tStart = t  # local t and not account for scr refresh
        end_text_8.tStartRefresh = tThisFlipGlobal  # on global time
        win.timeOnFlip(end_text_8, 'tStartRefresh')  # time at next scr refresh
        end_text_8.setAutoDraw(True)
    
    # *end_text_9* updates
    if end_text_9.status == NOT_STARTED and tThisFlip >= 0.0-frameTolerance:
        # keep track of start time/frame for later
        end_text_9.frameNStart = frameN  # exact frame index
        end_text_9.tStart = t  # local t and not account for scr refresh
        end_text_9.tStartRefresh = tThisFlipGlobal  # on global time
        win.timeOnFlip(end_text_9, 'tStartRefresh')  # time at next scr refresh
        end_text_9.setAutoDraw(True)
